...
def gen_species_spectrogram(train_test_split=False):
	for bird_type in glob.glob('../train_2_birds_wavs/*'):
		bird_name = os.path.basename(bird_type)
		print('Bird:', bird_name)
		if (len(os.listdir(bird_type)) > 0) and (bird_name + '.npy' not in os.listdir('../train_2_birds_spectrograms')):
			spec_samples = []
			...
			if spec_samples.shape[0] > 2 and train_test_split == True:
				...
				if train_spec_samples.shape[0] > 0:
					np.save('../train_2_birds_spectrograms/' + bird_name + '.npy', train_spec_samples)
				if test_spec_samples.shape[0] > 0:
					np.save('../test_2_birds_spectrograms/' + bird_name + '.npy', test_spec_samples)

			else:
				np.save('../train_2_birds_spectrograms/' + bird_name + '.npy', spec_samples)

			print('')	

def gen_single_bird_spectrogram(train_test_split=False):
	for bird_type in glob.glob('../train_2_birds_wavs/*'):
		for recording in glob.glob(bird_type + '/*'):
			bird_name = os.path.basename(recording).replace('.wav', '')
			print('Bird:', bird_name)
			if (len(os.listdir(bird_type)) > 0) and (bird_name + '.npy' not in os.listdir('../single_bird_spectrograms')):
				spec_samples = []
				print('\t - ', recording)
				try:
					# do vad stuff
				...
				if spec_samples.shape[0] > 2 and train_test_split == True:
					...
					if spec_samples.shape[0] > 0:
						np.save('../single_bird_spectrograms/' + bird_name + '.npy', spec_samples)

			print('')	
if __name__ == "__main__":
	#gen_species_spectrogram(train_test_split=False)
	gen_single_bird_spectrogram(train_test_split=False)
